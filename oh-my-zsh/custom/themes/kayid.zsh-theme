...
function venv_status() {
    if [[ "$VIRTUAL_ENV" != "" ]]; then
        echo "%{$fg_bold[green]%}VENV"
    else
  	echo "%{$fg_bold[red]%}VENV"
    fi
}

PROMPT='$nl$username_output$hostname_output$current_dir_output%1(j. [$jobs_bg].)'
GIT_PROMPT='$(out=$(git_prompt_info);if [[ -n $out ]]; then printf %s "$white on $purple$out$white [$(git_prompt_status)$(git_remote_status)$white] $(venv_status) $reset";fi)'
PROMPT+="$GIT_PROMPT"
PROMPT+='$nl'
PROMPT+='$yellow>>%{$reset_color%} '
